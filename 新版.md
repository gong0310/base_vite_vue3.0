...
​	1、一开始`jsx`语法经过babel编译,会被Babel编译为`React.createElement`方法,createElemen被调用转换成虚拟DOM，虚拟dom->真实dom，setstate数据改变的时候,会再生成一个虚拟dom,该虚拟dom与前一个虚拟dom通过diff算法**标记(就好比key)**(递归遍历)就行部分更新
...
setState接收两个参数，第一个：对象/函数 第二个：数据修改并且dom更新后的回调函数

**setstate是个异步， 可以调用多次，会集中一次更新**

如果要在原来状态的基础上进行操作的话，建议使用setState(函数)，对象会覆盖上一次执行



注意：18版本都是异步，在17版本中**在定时器 / Promise的情况下更新状态是同步的**，其他情况都是异步

```
如果是在原生js执行的空间：比如说setTimeout里面，setState是同步的，因为每次执行setState将立即更新this.state，然后触发render方法，渲染数据；
...
优点： 将异步与reducer区分开了，更加优雅，适合大量APi请求，而且每个请求之间存在复杂的依赖关系 缺点：学习曲线比较陡，理解async await；而且库也比较大，即使发布的最小也有25kb,gzip压缩后也有7KB,React压缩后才45kb
...
- ④Mobx-react vs     React-redux：redux采取Provider和connect方式；mobx采取Provider和inject、observer
...
1. 代码更简洁、可读性强、更容易复用代码

​	class 组件状态逻辑复用困难。只能通过 HOC 或者 render props，能会导致嵌套地狱

2. 不再需要考虑class组件中this指向

[缺点：](https://www.zhangshengrong.com/p/AvN6YvB8am/)

1. 状态不同步，函数的运行是独立的，每个函数都有一份独立的闭包作用域

​      可以通过useRef（current属性是可变的）来保存state进行解决
...
1. useEffect依赖引用类型可能会触发死循环

​			不要在useEffect里面写太多的依赖项，划分这些依赖项成多个单一功能的useEffect。其实这点是遵循了软件设计的“单一职责模式”。

2. 不能在条件、循环中或者嵌套函数中调用hooks，必须在react函数顶层使用hooks，因为react需要利用调用顺序来正确更新相应的状态，如果顺序打乱了，可能会出现预料不到的后果
3. 复杂业务的时候，使用Component代替hooks
4. React.memo并不能完全替代shouldComponentUpdate(因为拿不到 state change，只针对 props change)

### 13、事件绑定原理

 react并没有使用原生的浏览器事件，而是在基于VDOM的基础上实现了合成事件，采用小驼峰命名法，默认的事件传播方式是冒泡，如果想改为捕获的话，直接在事件名后面加上Capture即可；事件对象event也不是原生事件对象，而是合成对象，但通过nativeEvent属性可以访问原生事件对象；

> 所有事件均注册到了元素的最顶层-document 上 节点的事件由统一的入口处理 为了方便



采用合成事件的原因:

1. 统一不同浏览器事件的兼容处理和实现跨平台开发
2. 统一挂载在document上，减少内存的消耗，提升性能，方便在组件挂载/卸载时统一订阅和移除事件，
3. 方便事件统一管理
...
在该阶段主要做了两件事：document上注册、存储事件回调。**所有事件都会注册统一绑定到document上**，

> 因为这样React可以统一处理所有的事件绑定，在销毁的时候可以统一移除绑定

拥有统一的回调函数dispatchEvent来执行事件分发，类似于document.addEventListener("click",dispatchEvent)。
...
遍历AST，标记节点内容不变的节点为静态节点和静态根节点（被标记的节点在diff对比对时会跳过，从而提高虚拟dom比对的性能
...
防止从子组件意外改变父级组件的状态，导致数据流混乱，保证了数据可控性
...
### [Vue更新视图的数组方法有哪些？](https://www.jianshu.com/p/886ac356c13d)
...
具体：https://developer.aliyun.com/article/708459
...
// 创建一个新对象，该新对象的原型指向Array的原型。defineProperty进行拦截，将数组中会修改自身的方法进行重写
...
* 用户体验好, 速度块, 内容改变的时候不需要重新加载页面, 避免了不必要的跳转和重复渲染
* 前后端分离, 降低耦合度, 提升开发效率，对服务器压力较小
* 不以页面为单位, 更多的采用组件化的思想, 代码结构和组织方式更加规范化, 便于修改和调整

#### 缺点:

* 首次加载需要加载大量的资源, 加载时间相对较长，需要在加载页面的时候将 JavaScript、CSS 统一加载，部分页面按需加载；
* 不利于 SEO 优化, 单页页面, 数据在前端渲染，所有的内容都在一个页面中动态替换显示， 就意味着没有SEO
* 页面导航不可用, 如果一定要用导航前进, 后退，所有的页面切换需要自己建立堆栈管理
...
- SPA 页面采用**keep-alive缓存组件**，变量本地化
- 在更多的情况下，使用v-if替代v-show
- key保证唯一
- 使用路由懒加载、异步组件
- 防抖、节流
- 第三方模块按需导入，如vant、element-ui组件库
- 长列表滚动到可视区域动态加载
- 图片懒加载
...
- 多个 Mixin 之间可能产生冲突（比如定义了相同的data字段）
- 组件与 Mixin 之间存在隐式依赖，维护成本和理解成本迅速攀升，因为不敢轻易删改组件内数据，不确定有没有 Mixin 依赖它
...
**浏览器只保证 requestAnimationFrame 的回调在重绘之前执行，但没有确定的时间，何时重绘由浏览器决定**
...
**TCP/IP 中有两个具有代表性的[传输层协议](https://so.csdn.net/so/search?q=传输层协议&spm=1001.2101.3001.7020)，分别是 TCP（可靠的，面向连接的、基于字节流的传输层通信协议） 和 UDP。**
...
问题的根源在于，接收端不知道发送端将要传送的字节流的长度，**导致数据在传输时，在一条消息中读取到了另一条消息的部分数据**，所以解决粘包的方法就是围绕，如何让发送端在发送数据前，把自己将要发送的字节流总大小让接收端知晓，然后接收端来一个死循环接收完所有数据。
...
- cookie
  ...
  **注意：**
  ...
  浏览器来说，“cookie区分域，而不区分端口“，所以cookie在相同域名不同端口号可以共享
...
基于JSON格式的安全认证，跨域身份验证解决方案。他的两大使用场景是：认证和数据交换
...
**典型的用法**是加上`Authorization`header并使用Bear模式。Header的内容如下：
...
​			开销大：每个用户在认证之后，都要在服务端做一次记录，以方便该用户下次请求的鉴别。通常session保			存在内存中，随着认证用户的增多，服务端存储session的开销显著增大。

​			扩展性低：用户认证记录存储在认证服务器的内存中，这意味着用户下次请求仍要访问这台服务器才能拿			到授权。在分布式应用上，这限制了负载均衡的能力，进而限制了整个应用的扩展能力

##### 优点

- 因为json的通用性，所以JWT是可以进行跨语言支持的，像JAVA,JavaScript,NodeJS,PHP等很多语言都可以使用。
- 因为有了payload部分，所以JWT可以在自身存储一些其他业务逻辑所必要的非敏感信息。payload可以扩展
...
4. JWT 默认是不加密，但也是可以加密的，生成原始 Token 以后，可以用密钥再加密一次
5. JWT 不仅可以用于认证，也可以用于交换信息
...
当第二个过期的请求进来，token正在刷新，我们先将这个请求存到一个数组队列（重新发起请求）中，想办法让这个请求处于等待中，一直等到刷新token后再逐个重试清空请求队列。

- 问题三：那么如何做到让这个请求处于等待中呢？

为了解决这个问题，我们得借助Promise。将请求存进队列中后，同时返回一个Promise进行包裹，让这个Promise一直处于Pending状态（即不调用resolve），此时这个请求就会一直等啊等，只要我们不执行resolve，这个请求就会一直在等待。当刷新请求的接口返回来后，我们再调用resolve，逐个重试。
...
1）协商缓存（对比缓存）：流览器第一次请求数据时，服务器会将缓存标识与数据一起返回给客户端，客户端将二者备份至缓存数据库（电脑本地磁盘）中。再次请求数据时，客户端将备份的缓存标识发送给服务器，服务器根据缓存标识进行判断，判断成功后，返回**304**状态码，通知客户端比较成功，可以使用缓存数据
...
- 在 OSI 网络模型中，HTTPS的加密是在传输层完成的,因为SSL是位于传输层的,TLS的前身是SSL,所以同理
  - 它在 HTTP 和 TCP 的传输中建立了一个安全层，利用 对称加密 和 非对称加密 结合数字证书认证的方式，提高传输过程的安全性
...
- `403`: 拒绝请求
- `404`: 资源不存在
- `408`: 请求超时， 服务器等待了太长时间；服务器发送一个请求给客户端，客户端没有及时回应，服务器会判断连接已断开
- `500`: 服务器错误
...
应用场景：提交按钮，计算鼠标移动距离，监听scroll滚动事件。
...
恶意攻击者可以通过Web页面中插入一些恶意内容（Script代码）。当用户浏览该页面的时候，那么嵌入到web页面中Script代码会执行，从而达到恶意攻击用户的目的。

恶意代码执行后，就能获取到用户数据，比如上面的cookie等信息，那么把该cookie发送到攻击者网站中，那么攻击者拿到该cookie然后会冒充该用户的行为，调用目标网站接口等违法操作。
...
> 这种攻击常见于带**有用户保存数据的网站功能**，如论坛发帖、博客网站、商品评论、用户私信等。前端可以进行一些处理，比如将script标签等特殊字符替换成HTML 转义
...
- 用正则对输入进行过滤，检查是否有非法字符,如<>（尖括号）、”（引号）、 ‘（单引号）、%（百分比符号
- 对输入内容转码或者过滤，让其不可执
- 利用 HttpOnly 

  - 很多 XSS 攻击脚本都是用来窃取Cookie, 而设置 Cookie 的 HttpOnly 属性后，JavaScript 便无法读取 Cookie 的值。这样也能很好的防范 XSS 攻击
  - **（最佳答案：原理上是将特殊字符转义成HTML实体，实践上是使用vue/react默认方式渲染）**

  **（回答提交时在客户端阻止、替换半角为全角、服务端替换等都不是最优解）**

> 在服务器端设置cookie的时候设置`HttpOnly`, 这样就可以防止用户通过Document.cookie获取cookie。此预防措施有助于**缓解**跨站点脚本（XSS）攻击。



#### CSRF/XSRF（跨站请求伪造）

利用用户登录信息尚未过期的登录凭证（Cookie），欺骗用户浏览器，让其以用户的名义运行操作。

> 1、用户登录一家银行用以运行转账  a.com
>
> 2、攻击者引诱受害者访问了b.com并放置如下代码：
> `<img src="a.com?account=Alice&amount=1000&for=Badman">`
>
...
- 检查Referer字段
  ...
  HTTP头中有一个Referer字段，这个字段用以标明请求来源于哪个地址。然后服务端校验判断Referer字段应和请求的地址位于同一域名下。
...
**CDN**(内容分发网络），将源站的资源缓存到分布在全国各地的CDN节点。用户请求数据时，先访问距离用户最近的CDN节点上缓存的资源，没有取到缓存的时候才去访问源站，缩短了用户等待时长，为源站分担了压力，这是一项用存储空间换取时间的技术
...
#### [**EventLoop运行机制**](https://www.cnblogs.com/luoxiaoer/p/11751810.html)

**宏任务是由宿主（浏览器、Node）发起的，而微任务由 JS 自身发起。**

所有的同步任务都是在主进程执行的形成一个执行栈，主线程之外，还存在一个"任务队列"，异步任务执行队列中先执行宏任务，然后清空当次宏任务中的所有微任务，然后进行下一个tick如此形成循环。**最后执行 UI 渲染 操作**
...
根据**html元素(根元素)font-size计算**的**倍数**的**css单位**
...
设计尺寸750 根元素为100px  便于计算
```

虚拟列表

vitehttps://www.knowbaike.com/it/13131.html

- 轻量快速的热重载（HMR），不会随项目变大而变慢，当某个模块内容改变时，让浏览器去重新请求该模块即可，

   而不是像webpack重新将该模块的所有依赖重新编译

- 只需要初始化，开箱即用，内置CSS预处理、html预处理、异步加载、分包、压缩、HMR等

- dev环境采用（原生JS module(ESM)，(<script type="module">)）不需要打包，只需处理一些源代码的路径问题和预构建
...
[自动化构建、测试、分析、部署的流水线方案](https://demo.dandelioncloud.cn/article/details/1567447665543966721)

主要工具：

- gitlab 用于代码版本管理，并通过其提供的 webhook ，触发 jenkins job 的运行。
...
思想：保证一个类只有一个实例，并提供一个全局访问点。实现方法：先判断实例存在与否，如果存在则直接返回，如果不存在就创建了再返回，这就确保栈了一个类只有一个实例对象。
...
- sendBeacon通过 HTTP POST方法发送请求， 将少量数据异步传输，可靠性好
...
![](https://img-blog.csdnimg.cn/bb5068fe2e474760bec344da915004ff.png)

![](https://img-blog.csdnimg.cn/20200612171601954.jpg?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3g1NTAzOTIyMzY=,size_16,color_FFFFFF,t_70)
